...
module.exports = async function (req, res, next) {
	...
	try {
		...
		if (Boolean(user)) {
			...
			if (user.userRole == constants.USER_ROLE_STAFF) {
				req.user = decoded;
				next();
			}
			else {
				return res.status(400).json({ success: false, message: "Staff Permision denied." })
			}
		}
		else {
		...
		//if invalid token
		config.logger.error({ ex }, 'Error in staffAuth');
