...
const permutations = arr => {
	if (arr.length <= 2) return arr.length === 2 ? [arr, [arr[1], arr[0]]] : arr; return arr.reduce(
		(acc, item, i) =>
			acc.concat(permutations([...arr.slice(0, i), ...arr.slice(i + 1)]).map(val => [
				item, ...val,
			])
			),
		[]
...
exports.canCleanupCollectionData = function (startDate, endDate, month = 3) {
	const dateFormat = "YYYY-MM-DD HH:mm:ss";
	const startDateMoment = moment(startDate).format(dateFormat);
	const endDateMoment = moment(endDate).format(dateFormat);
	const beforeNmonthsMoment = moment().subtract(month, 'months').format(dateFormat);
	const startDateYear = moment(startDateMoment).format('YYYY');
	const startDateMonth = moment(startDateMoment).format('MM');
	const startDateDay = moment(startDateMoment).format('DD');
	const endDateYear = moment(endDateMoment).format('YYYY');
	const endDateMonth = moment(endDateMoment).format('MM');
	const endDateDay = moment(endDateMoment).format('DD');
	const beforeNmonthsYear = moment(beforeNmonthsMoment).format('YYYY');
	const beforeNmonthsMonth = moment(beforeNmonthsMoment).format('MM');
	const beforeNmonthsDay = moment(beforeNmonthsMoment).format('DD');
	const isValidDates = Number(endDateYear.toString() + endDateMonth.toString() + endDateDay.toString()) - Number(startDateYear.toString() + startDateMonth.toString() + startDateDay.toString()) > 0;
	const isValidEndDate = Number(beforeNmonthsYear.toString() + beforeNmonthsMonth.toString() + beforeNmonthsDay.toString()) - Number(endDateYear.toString() + endDateMonth.toString() + endDateDay.toString()) > 0;
	return isValidDates && isValidEndDate;
}

exports.extractByGameDate = function (dataArray) {
	let grouped = _.groupBy(dataArray, function (item) {
		return item.date;
	});
	return grouped;
}
exports.extractByGameType = function (dataArray) {
	let grouped = _.groupBy(dataArray, function (item) {
		return item.gameType;
	});
	return grouped;
}
exports.extractByPlayingGameType = function (dataArray) {
	let grouped = _.groupBy(dataArray, function (item) {
		return item.playingGameType;
	});
	return grouped;
}

exports.extractBySelectedValue = function (dataArray) {
	let grouped = _.groupBy(dataArray, function (item) {
...
exports.getRandomValue = function (arr) {
	return arr[Math.floor(Math.random() * arr.length)];
}

exports.getTimeSlotNumber = function () {
	const dateFormat = "YYYY-MM-DD HH:mm:ss";
	const rightNow = moment().utc().format(dateFormat);
	const midNightThisDay = moment().startOf('day').format(dateFormat);
	let totalMinutes = moment(rightNow).diff(midNightThisDay, 'minutes');
	let month = moment(rightNow).format('MM').toString();
	let day = moment(rightNow).format('DD').toString();
	let year = moment(rightNow).format('YYYY').toString();
	let slotNumber = year + month + day + parseInt(parseInt(totalMinutes) / 2).toString();
	return slotNumber;
};

exports.getLotteryTicketNumber = function (lotteryType) {
	const dateFormat = "YYYY-MM-DD HH:mm:ss";
	const rightNow = moment().utc().format(dateFormat);
	const midNightThisDay = moment().startOf('day').format(dateFormat);
	let totalSeconds = moment(rightNow).diff(midNightThisDay, 'seconds');
	let month = moment(rightNow).format('MM').toString();
	let day = moment(rightNow).format('DD').toString();
	let year = moment(rightNow).format('YYYY').toString();
	let ticketNumber = year + month + day + Math.abs(totalSeconds || 0).toString();
	let preFix = "";
	switch (lotteryType) {
		case constants.LOTTERY_GAME_TYPE_THAILAND: preFix = "TH"; break;
		case constants.LOTTERY_GAME_TYPE_BANGKOK_WEEKLY: preFix = "BW"; break;
		case constants.LOTTERY_GAME_TYPE_DUBAI_DAILY: preFix = "DD"; break;
		case constants.LOTTERY_GAME_TYPE_LONDON_WEEKLY: preFix = "LW"; break;
		case constants.LOTTERY_GAME_TYPE_MEXICO_MONTHLY: preFix = "MM"; break;
		default: break;
	}
	return preFix + ticketNumber;
};

exports.generate3UPCombinations = function (num) {
	let numArr = num.toString().split('');
	let ret = [];
	ret.push(numArr[0] + numArr[1] + numArr[2]);
	ret.push(numArr[0] + numArr[2] + numArr[1]);
	ret.push(numArr[1] + numArr[2] + numArr[0]);
	ret.push(numArr[1] + numArr[0] + numArr[2]);
	ret.push(numArr[2] + numArr[0] + numArr[1]);
	ret.push(numArr[2] + numArr[1] + numArr[0]);
	return [...new Set(ret)];
}

exports.generateNumberCombinations = function (num) {
	const numArr = num.toString().split("");
	const arr = permutations(numArr);
	const result = arr.map(x => x.join(""));
	return [...new Set(result)];
}

exports.generateReferralCode = function (length) {
	let result = '';
...
exports.parseLotteryGameNumber = function (gameNumber) {
...
exports.getLastGameWinnersList = function (count) {
	// calculate time left
	let UserNames = [
...
exports.calculateLotteryGameDiscounts = function (isSpecialUser, lotteryGameType, totalPlayedAmount, gameType, gameSettings, permissionsObj, currDateTimeObj, lotteryGameParserObj) {
	let discount = 0;
...
const func = {
	...
	generateToken: (payload) => {
		...
		return func.generateRandomString(32); // Placeholder
	},

	// Verify JWT token (placeholder)
	verifyToken: (token) => {
		// In production, use jsonwebtoken.verify
		return { valid: true, payload: {} }; // Placeholder
	},
	...
	sanitizeObject: (obj) => {
		...
		for (const [key, value] of Object.entries(obj)) {
			if (typeof value === 'string') {
				sanitized[key] = func.sanitizeString(value);
			} else if (typeof value === 'object' && value !== null) {
				sanitized[key] = func.sanitizeObject(value);
			} else {
				sanitized[key] = value;
			}
		}
	...
	deepClone: (obj) => {
		...
		if (obj instanceof Array) return obj.map(item => func.deepClone(item));
		if (typeof obj === 'object') {
			const clonedObj = {};
			for (const key in obj) {
				if (obj.hasOwnProperty(key)) {
					clonedObj[key] = func.deepClone(obj[key]);
				}
			}
			return clonedObj;
		}
